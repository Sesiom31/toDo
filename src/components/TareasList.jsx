...
export function Tarea({ tarea, onImportant, onCompleted }) {
  ...
  return (
    <li
      className="flex justify-between items-center w-auto h-16  shadow-inner shadow-gray-500 
    ...
    >
      <div className="flex items-center gap-4 md:gap-20 h-full">
        ...
        <div className=" flex flex-col justify-center gap-2 h-full">
          ...
          <p className=" text-xs font-semibold text-gray-500 flex gap-4">
            <span>{formatDate(new Date(tarea.date_start))}</span>
            {tarea.date_end !== '' && (
              <>
                <span> - </span>
                <span>{formatDate(new Date(tarea.date_end))}</span>
              </>
            )}
          </p>{' '}
        </div>
